-- Product expansion authorization alignment. Drafts remain Customer-only and
-- free text never authorizes a Match.
      ...
      AND (
        (
          ...
          AND EXISTS (
            ...
            WHERE service_selection.supplier_id = _supplier_id
              AND service_selection.service_id = request_row.service_id
          )
        )
        OR
        (
        ...
        (
          ...
          AND NOT EXISTS (
            ...
            WHERE governed_profession.category_id = request_row.category_id
          ...
          AND EXISTS (
            ...
            WHERE category_selection.supplier_id = _supplier_id
              AND category_selection.category_id = request_row.category_id
          )
        )
      )
  );
...
ALTER FUNCTION public._supplier_matches_request_taxonomy(uuid, uuid)
  OWNER TO postgres;
REVOKE ALL ON FUNCTION public._supplier_matches_request_taxonomy(uuid, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._supplier_matches_request_taxonomy(uuid, uuid)
  TO service_role;
...
BEGIN
  ...
  IF _request.id IS NULL
    ...
    OR _request.published_at IS NULL THEN
    RETURN 0;
  END IF;
  ...
  FOR _row IN
    WITH candidates AS (
      ...
      WHERE _request.service_id IS NULL
        ...
        AND profession_selection.subcategory_id = _request.subcategory_id
      ...
      WHERE _request.service_id IS NULL
        ...
        AND NOT EXISTS (
          SELECT 1
          FROM public.subcategories governed_profession
          WHERE governed_profession.category_id = _request.category_id
            AND governed_profession.is_active
        )
    ),
    inserted AS (
...
CREATE OR REPLACE FUNCTION public._generate_matches_for_supplier_subcategory(
  _supplier_id uuid,
...
BEGIN
  ...
  FOR _row IN
    WITH inserted AS (
      ...
      WHERE request_row.status = 'open'
        AND request_row.published_at IS NOT NULL
        AND request_row.service_id IS NULL
        AND request_row.subcategory_id = _subcategory_id
      ON CONFLICT (supplier_id, request_id)
      DO UPDATE SET
        status = 'active',
        updated_at = now()
...
ALTER FUNCTION public._generate_matches_for_supplier_subcategory(uuid, uuid)
  OWNER TO postgres;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.status = 'open' AND NEW.published_at IS NOT NULL THEN
...
CREATE TRIGGER requests_matches_au
  AFTER UPDATE OF status, published_at ON public.requests
  FOR EACH ROW EXECUTE FUNCTION public.trg_requests_matches();
...
AS $$
BEGIN
  PERFORM public._generate_matches_for_supplier(NEW.supplier_id);
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER supplier_services_matches_ai
  AFTER INSERT ON public.supplier_services
  FOR EACH ROW EXECUTE FUNCTION public.trg_supplier_services_ai();
...
BEGIN
  ...
  WHERE match_row.request_id = request_row.id
    AND match_row.supplier_id = OLD.supplier_id
    AND match_row.status = 'active'
    AND request_row.service_id = OLD.service_id;
...
CREATE TRIGGER supplier_services_matches_ad
  AFTER DELETE ON public.supplier_services
  FOR EACH ROW EXECUTE FUNCTION public.trg_supplier_services_ad();
...
CREATE TRIGGER supplier_service_areas_matches_ad
  AFTER DELETE ON public.supplier_service_areas
  FOR EACH ROW EXECUTE FUNCTION public.trg_supplier_service_areas_ad();
...
CREATE TRIGGER supplier_categories_matches_ai
  AFTER INSERT ON public.supplier_categories
  FOR EACH ROW EXECUTE FUNCTION public.trg_supplier_categories_ai();

-- The Gate 6 projection remains the only Supplier Request read surface. It now
-- carries governed Service context and only the answers for this Request.
...
BEGIN
  ...
  END IF;
  IF NOT public._is_supplier_profile_complete(_supplier_id) THEN
    RAISE EXCEPTION 'Supplier profile is incomplete'
      USING ERRCODE = '22000';
  END IF;
...
BEGIN
  ...
  WHERE match_row.status = 'active'
    AND (
      NOT public._is_supplier_profile_complete(match_row.supplier_id)
      OR NOT public._supplier_matches_request_taxonomy(
