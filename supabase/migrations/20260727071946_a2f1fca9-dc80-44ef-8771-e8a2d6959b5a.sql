...
CREATE TRIGGER validate_request_biu BEFORE INSERT OR UPDATE ON public.requests FOR EACH ROW EXECUTE FUNCTION public.validate_request();
CREATE TRIGGER guard_request_update_bu BEFORE UPDATE ON public.requests FOR EACH ROW EXECUTE FUNCTION public.guard_request_update();
CREATE TRIGGER validate_offer_biu BEFORE INSERT OR UPDATE ON public.offers FOR EACH ROW EXECUTE FUNCTION public.validate_offer();
CREATE TRIGGER guard_offer_update_bu BEFORE UPDATE ON public.offers FOR EACH ROW EXECUTE FUNCTION public.guard_offer_update();
CREATE TRIGGER sync_request_offers_count_aiud AFTER INSERT OR UPDATE OF status OR DELETE ON public.offers FOR EACH ROW EXECUTE FUNCTION public.sync_request_offers_count();
CREATE TRIGGER on_offer_awarded_au AFTER UPDATE OF status ON public.offers FOR EACH ROW EXECUTE FUNCTION public.on_offer_awarded();
...
BEGIN
  ...
  IF NEW.description <> OLD.description AND OLD.offers_count > 0 THEN RAISE EXCEPTION 'Description cannot change after offers arrive'; END IF;
...
RETURNS trigger LANGUAGE plpgsql SET search_path = public AS $$
DECLARE
  is_customer boolean; is_supplier boolean; content_changed boolean;
BEGIN
  ...
  is_customer := EXISTS (SELECT 1 FROM public.requests request_row WHERE request_row.id = OLD.request_id AND request_row.customer_id = auth.uid());
  content_changed := NEW.price <> OLD.price OR NEW.estimated_days <> OLD.estimated_days OR NEW.message <> OLD.message;
  IF NEW.status <> OLD.status THEN
    IF content_changed THEN RAISE EXCEPTION 'Offer content cannot change during a status transition'; END IF;
    IF is_supplier AND NEW.status = 'withdrawn' AND OLD.status = 'submitted' THEN NEW.withdrawn_at := now();
    ELSIF is_customer AND NEW.status = 'selected' AND OLD.status = 'submitted' AND EXISTS (SELECT 1 FROM public.requests request_row WHERE request_row.id = OLD.request_id AND request_row.status = 'open') THEN NULL;
    ELSIF current_user = 'postgres' AND NEW.status = 'rejected' AND OLD.status = 'submitted' THEN NULL;
    ELSE RAISE EXCEPTION 'Illegal offer status transition from % to %', OLD.status, NEW.status;
    END IF;
  ELSE
    IF NEW.withdrawn_at IS DISTINCT FROM OLD.withdrawn_at THEN RAISE EXCEPTION 'withdrawn_at is system-managed'; END IF;
    IF content_changed THEN
      IF NOT is_supplier THEN RAISE EXCEPTION 'Only the supplier may edit offer content'; END IF;
      IF OLD.status <> 'submitted' THEN RAISE EXCEPTION 'Offer content is immutable after status leaves submitted'; END IF;
    END IF;
  END IF;
  RETURN NEW;
END;
...
SET offers_count = (SELECT count(*)::integer FROM public.offers offer_row WHERE offer_row.request_id = request_row.id AND offer_row.status = 'submitted')
WHERE request_row.offers_count IS DISTINCT FROM (SELECT count(*)::integer FROM public.offers offer_row WHERE offer_row.request_id = request_row.id AND offer_row.status = 'submitted');
...
WHERE match_row.request_id = request_row.id
  ...
  AND (request_row.status <> 'open' OR request_row.subcategory_id IS NULL
       OR NOT public._is_supplier_profile_complete(match_row.supplier_id)
       OR NOT public._supplier_serves_subcategory(match_row.supplier_id, request_row.subcategory_id));

CREATE OR REPLACE FUNCTION public.admin_create_match(_supplier_id uuid, _request_id uuid)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  _match_id uuid; _request_status public.request_status; _request_subcategory uuid;
BEGIN
  ...
  SELECT status, subcategory_id INTO _request_status, _request_subcategory FROM public.requests WHERE id = _request_id;
  IF _request_status IS NULL THEN RAISE EXCEPTION 'Unknown request' USING ERRCODE = '22000'; END IF;
  IF _request_status <> 'open' THEN RAISE EXCEPTION 'Request is not open' USING ERRCODE = '22000'; END IF;
  IF _request_subcategory IS NULL THEN RAISE EXCEPTION 'Request has no matchable subcategory' USING ERRCODE = '22000'; END IF;
  ...
  IF NOT public._supplier_serves_subcategory(_supplier_id, _request_subcategory) THEN RAISE EXCEPTION 'Supplier does not serve this subcategory' USING ERRCODE = '22000'; END IF;
