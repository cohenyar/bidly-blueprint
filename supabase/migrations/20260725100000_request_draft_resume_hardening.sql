-- Make draft acquisition atomic across refreshes, Strict Mode, and concurrent
-- tabs. Existing published Requests are unaffected.

CREATE UNIQUE INDEX requests_one_draft_per_customer_idx
...
BEGIN
  ...
  -- Transaction-scoped and caller-specific: only one tab can create the
  -- Customer's draft, while unrelated Customers remain concurrent.
  PERFORM pg_advisory_xact_lock(
    hashtextextended(auth.uid()::text, 20260725)
  );
  ...
  ORDER BY request_row.updated_at DESC
  LIMIT 1;

  IF _request_id IS NULL THEN
    INSERT INTO public.requests (customer_id)
    VALUES (auth.uid())
...
REVOKE ALL ON FUNCTION public.get_or_create_request_draft()
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_or_create_request_draft()
  TO authenticated, service_role;

-- Drafts may contain a partially entered budget while autosave runs. Strict
-- coherence remains mandatory as soon as published_at is set.
ALTER TABLE public.requests
  ...
  ADD CONSTRAINT requests_missing_service_text_check
    CHECK (
      ...
      OR (
        char_length(missing_service_text) <= 500
        AND (
          published_at IS NULL
          OR char_length(btrim(missing_service_text)) >= 3
        )
      )
    ) NOT VALID;
...
BEGIN
  ...
  IF NEW.title IS NOT NULL
    AND (
      (NEW.published_at IS NOT NULL AND char_length(btrim(NEW.title)) < 3)
  ...
  IF NEW.city IS NOT NULL
    AND (
      (NEW.published_at IS NOT NULL AND char_length(btrim(NEW.city)) < 2)
  ...
  ELSIF NEW.budget_type = 'range' THEN
    IF _strict_budget
      OR (NEW.budget_min IS NOT NULL AND NEW.budget_max IS NOT NULL) THEN
      IF NEW.budget_min IS NULL
        OR NEW.budget_max IS NULL
        OR NEW.budget_min <= 0
        OR NEW.budget_max <= 0
...
-- A dependent question is hidden until its controlling answer exists. This
-- keeps `not_equals` conditions from becoming visible on an unanswered parent.
  ...
  SELECT EXISTS (
    ...
    WHERE request_row.id = _request_id
      AND (
        ...
        OR (
          dependency_answer.question_id IS NOT NULL
          AND (
            (
            ...
            OR (
              ...
              AND dependency_answer.answer IS DISTINCT FROM
...
-- Answer writes invoke the private visibility helper from this trigger. Run
-- the validator with the owning role so authenticated customers do not need
-- direct EXECUTE access to that helper.
ALTER FUNCTION public.validate_request_question_answer()
  SECURITY DEFINER;
ALTER FUNCTION public.validate_request_question_answer()
  SET search_path = public;
ALTER FUNCTION public.validate_request_question_answer()
...
-- Customers can continue to read the prompt attached to their own saved
-- answer if an administrator later deactivates that definition.
CREATE POLICY "Customers view own answered Request questions"
  ...
  USING (
    EXISTS (
      ...
      JOIN public.requests request_row
        ON request_row.id = answer_row.request_id
      WHERE answer_row.question_id = request_questions.id
